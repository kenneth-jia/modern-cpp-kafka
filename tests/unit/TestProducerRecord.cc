...
TEST(ProducerRecord, WithSpecificPartition)
{
    ...
    const kafka::Key key = kafka::Key(keyStr);
    const std::string payload = "hello world";
    const kafka::Value value = kafka::Value(payload);
    ...
    record.headers().emplace_back("hk1", kafka::Header::Value(headerValue1));
    record.headers().emplace_back("hk2", kafka::Header::Value(headerValue2));

    EXPECT_EQ("topic1-1: headers[hk1:hv1,hk2:hv2], key1/hello world", record.toString());

    const std::string payload2 = "it has been changed";
    record.setValue(kafka::Value(payload2));
